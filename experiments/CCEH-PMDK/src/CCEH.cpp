...
TOID(struct Segment)* Segment::Split(PMEMobjpool* pop){
    ...
    PMEM_POBJ_ALLOC(pop, &split[1], struct Segment, sizeof(struct Segment), NULL, NULL);
...
#else
    TOID(struct Segment)* split = new TOID(struct Segment)[2];
    PMEM_POBJ_ALLOC(pop, &split[0], struct Segment, sizeof(struct Segment), NULL, NULL);
    PMEM_POBJ_ALLOC(pop, &split[1], struct Segment, sizeof(struct Segment), NULL, NULL);
...
void CCEH::initCCEH(PMEMobjpool* pop){
    ...
    PMEM_POBJ_ALLOC(pop, &D_RW(dir)->segment, TOID(struct Segment), sizeof(TOID(struct Segment))*D_RO(dir)->capacity, NULL, NULL);
...
DIR_RETRY:
	...
	PMEM_POBJ_ALLOC(pop, &_dir, struct Directory, sizeof(struct Directory), NULL, NULL);
	PMEM_POBJ_ALLOC(pop, &D_RO(_dir)->segment, TOID(struct Segment), sizeof(TOID(struct Segment))*D_RO(dir)->capacity*2, NULL, NULL);
