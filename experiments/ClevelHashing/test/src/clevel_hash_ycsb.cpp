...
#define READ_WRITE_NUM 640000

#else

// (2^12 + 2^11) * 8 = 49152
#define HASH_POWER 12
#define READ_WRITE_NUM 160000
...
nvobj::pool<root> create_object(const char *path, const char *layout, size_t poolsize, mode_t mode){
	nvobj::pool<root> pop = nvobj::pool<root>::create(path, layout, poolsize, mode);
	PMEMobjpool* pool_handle = pop.handle();
	pmem_create((unsigned long long)pool_handle, poolsize);
        return pop;
	}
...
//		path, LAYOUT, PMEMOBJ_MIN_POOL * 1024, S_IWUSR | S_IRUSR);
	...
	while(getline(&pbuf,&len,ycsb_read) != -1){
		// printf("%d, %d, %d\n", operation_num%thread_num, move[operation_num%thread_num], operation_num);
		if(strncmp(buf, "INSERT", 6) == 0){
    ...
    for(size_t t = 0; t < thread_num; t++){
		...
		printf("For thread:%d, run queue is:%d\n", t, t);
	...
	for (size_t i = 0; i < thread_num; i++)
	{
		threads.emplace_back([&](size_t thread_id) {
			...
			size_t offset = loaded + operation_num / thread_num * thread_id;
			for (size_t j = 0; j < operation_num / thread_num; j++)
			{
//				printf("%d, %d\n", thread_id, j);
