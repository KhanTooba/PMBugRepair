 ...
 */
__attribute__((used)) enum cc_stat cc_array_new(CC_Array **out)
{
    CC_ArrayConf c;
 ...
 */
__attribute__((used)) enum cc_stat cc_array_new_conf(CC_ArrayConf const * const conf, CC_Array **out)
{
    float ex;
    ...
    simuTX_BEGIN();
    simuTX_ADD(&ar->buffer);
    simuTX_ADD(&ar->exp_factor);
    simuTX_ADD(&ar->capacity);
    simuTX_ADD(&ar->mem_alloc);
    simuTX_ADD(&ar->mem_calloc);
    simuTX_ADD(&ar->mem_free);
 ...
 */
__attribute__((used)) void cc_array_conf_init(CC_ArrayConf *conf)
{
    simuTX_BEGIN();
    simuTX_ADD(&conf->exp_factor);
    simuTX_ADD(&conf->capacity);
    simuTX_ADD(&conf->mem_alloc);
    simuTX_ADD(&conf->mem_calloc);
    simuTX_ADD(&conf->mem_free);
    conf->exp_factor = DEFAULT_EXPANSION_FACTOR;
    conf->capacity   = DEFAULT_CAPACITY;
    conf->mem_alloc  = pmalloc;
    conf->mem_calloc = pcalloc;
    conf->mem_free   = pfree;
    simuTX_END();
}
 ...
 */
__attribute__((used)) void cc_array_destroy(CC_Array *ar)
{
 ...
 */
__attribute__((used)) void cc_array_destroy_cb(CC_Array *ar, void (*cb) (void*))
{
    size_t i;
    for (i = 0; i < ar->size; i++)
        cb(ar->buffer[i]);
 ...
 */
__attribute__((used)) enum cc_stat cc_array_add(CC_Array *ar, void *element)
{
    ...
    ar->size++;
    simuFlushOpt(&ar->buffer[ar->size], sizeof(ar->buffer[ar->size]));
    simuSfence();
    simuFlushOpt(&ar->size, sizeof(ar->size));
    simuSfence();
    return CC_OK;
}
 ...
 * array is already at maximum capacity.
 */
__attribute__((used)) enum cc_stat cc_array_add_at(CC_Array *ar, void *element, size_t index)
{
    if (index == ar->size)
...
enum cc_stat cc_array_replace_at(CC_Array *ar, void *element, size_t index, void **out)
{
    ...
    ar->buffer[index] = element;
    simuFlushOpt(&ar->buffer[index], sizeof(ar->buffer[index]));
    simuSfence();

    return CC_OK;
}

__attribute__((used)) enum  cc_stat cc_array_swap_at(CC_Array *ar, size_t index1, size_t index2)
{
    void *tmp;
    ...
    simuFlushOpt(&ar->buffer[index1], sizeof(ar->buffer[index1]));
    simuFlushOpt(&ar->buffer[index2], sizeof(ar->buffer[index2]));
    simuSfence();
    return CC_OK;
}
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_remove(CC_Array *ar, void *element, void **out)
{
    size_t index;
 ...
 * if the index was out of range.
 */
__attribute__((used)) enum  cc_stat cc_array_remove_at(CC_Array *ar, size_t index, void **out)
{
    if (index >= ar->size)
        return CC_ERR_OUT_OF_RANGE;
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_remove_last(CC_Array *ar, void **out)
{
 ...
 */
__attribute__((used)) void cc_array_remove_all(CC_Array *ar)
{
    ar->size = 0;
    simuFlushOpt(&ar->size, sizeof(ar->size));
    simuSfence();
 ...
 * @param[in] ar array from which all elements are to be removed
 */
__attribute__((used)) void cc_array_remove_all_free(CC_Array *ar)
{
    size_t i;
    for (i = 0; i < ar->size; i++)
        free(ar->buffer[i]);
 ...
 * was out of range.
 */
__attribute__((used)) enum  cc_stat cc_array_get_at(CC_Array *ar, size_t index, void **out)
{
    if (index >= ar->size)
        return CC_ERR_OUT_OF_RANGE;
 ...
 * CC_Array is empty.
 */
__attribute__((used)) enum  cc_stat cc_array_get_last(CC_Array *ar, void **out)
{
    if (ar->size == 0)
        return CC_ERR_VALUE_NOT_FOUND;
 ...
 */
__attribute__((used)) const void * const* cc_array_get_buffer(CC_Array *ar)
{
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_index_of(CC_Array *ar, void *element, size_t *index)
{
    size_t i;
    for (i = 0; i < ar->size; i++) {
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_subarray(CC_Array *ar, size_t b, size_t e, CC_Array **out)
{
    if (b > e || e >= ar->size)
    ...
    if (!(sub_ar->buffer = ar->mem_alloc(ar->capacity * sizeof(void*)))) {
        ar->mem_free(sub_ar);
        simuFlushOpt(&sub_ar->buffer, sizeof(sub_ar->buffer));
        simuSfence();
        return CC_ERR_ALLOC;
    }
    simuFlushOpt(&sub_ar->buffer, sizeof(sub_ar->buffer));
    ...
    simuFlushOpt(&sub_ar->mem_alloc, sizeof(sub_ar->mem_alloc));
    simuFlushOpt(&sub_ar->mem_calloc, sizeof(sub_ar->mem_calloc));
    simuFlushOpt(&sub_ar->mem_free, sizeof(sub_ar->mem_free));
    simuFlushOpt(&sub_ar->size, sizeof(sub_ar->size));
    simuSfence();
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_copy_shallow(CC_Array *ar, CC_Array **out)
{
    ...
    simuFlushOpt(&copy->exp_factor, sizeof(copy->exp_factor));
    simuFlushOpt(&copy->size, sizeof(copy->size));
    simuFlushOpt(&copy->capacity, sizeof(copy->capacity));
    simuFlushOpt(&copy->mem_alloc, sizeof(copy->mem_alloc));
    simuFlushOpt(&copy->mem_calloc, sizeof(copy->mem_calloc));
    simuFlushOpt(&copy->mem_free, sizeof(copy->mem_free));
    simuFlushOpt(&copy->size, sizeof(copy->size));
    simuSfence();
    memcpy(copy->buffer,
           ar->buffer,
           copy->size * sizeof(void*));
    simuFlushOpt(&copy->buffer, sizeof(copy->buffer));
    simuSfence();
    *out = copy;
    return CC_OK;
}
 ...
 * memory allocation for the copy failed.
 */
__attribute__((used)) enum  cc_stat cc_array_copy_deep(CC_Array *ar, void *(*cp) (void *), CC_Array **out)
{
    CC_Array *copy = ar->mem_alloc(sizeof(CC_Array));
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_filter_mut(CC_Array *ar, bool (*pred) (const void*))
{
    if (ar->size == 0)
        return CC_ERR_OUT_OF_RANGE;
        ...
        if (rm > 0) {
            if (keep > 0) {
                ...
                memmove(&(ar->buffer[i + 1]),
                        &(ar->buffer[i + 1 + rm]),
                        block_size);
                simuFlushOpt(&ar->buffer[i + 1], sizeof(ar->buffer[i + 1]));
                simuSfence();
            }
            ar->size -= rm;
            simuFlushOpt(&ar->size, sizeof(ar->size));
    ...
    if (rm > 0) {
        ...
        memmove(&(ar->buffer[0]),
                &(ar->buffer[rm]),
                block_size);
        simuFlushOpt(&ar->buffer[0], sizeof(ar->buffer[0]));
        simuSfence();
        ar->size -= rm;
        simuFlushOpt(&ar->size, sizeof(ar->size));
        simuSfence();
 ...
 * new CC_Array failed.
 */
__attribute__((used)) enum  cc_stat cc_array_filter(CC_Array *ar, bool (*pred) (const void*), CC_Array **out)
{
    if (ar->size == 0)
        return CC_ERR_OUT_OF_RANGE;
    ...
    simuFlushOpt(&filtered->exp_factor, sizeof(filtered->exp_factor));
    simuFlushOpt(&filtered->size, sizeof(filtered->size));
    simuFlushOpt(&filtered->capacity, sizeof(filtered->capacity));
    simuFlushOpt(&filtered->mem_alloc, sizeof(filtered->mem_alloc));
    simuFlushOpt(&filtered->mem_calloc, sizeof(filtered->mem_calloc));
    simuFlushOpt(&filtered->mem_free, sizeof(filtered->mem_free));
    simuFlushOpt(&filtered->size, sizeof(filtered->size));
    simuSfence();
    size_t f = 0;
    for (size_t i = 0; i < ar->size; i++) {
        if (pred(ar->buffer[i])) {
            ...
            filtered->size++;
            simuFlushOpt(&filtered->buffer[f], sizeof(filtered->buffer[f]));
            simuFlushOpt(&filtered->size, sizeof(filtered->size));
            simuSfence();
        }
    }
 ...
 */
__attribute__((used)) void cc_array_reverse(CC_Array *ar)
{
    if (ar->size == 0)
        return;
    ...
    for (i = 0, j = ar->size - 1; i < ar->size / 2; i++, j--) {
        ...
        ar->buffer[j] = tmp;
        simuFlushOpt(&ar->buffer[i], sizeof(ar->buffer[i]));
        simuFlushOpt(&ar->buffer[j], sizeof(ar->buffer[j]));
        simuSfence();
    }
}
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_trim_capacity(CC_Array *ar)
{
    if (ar->size == ar->capacity)
        return CC_OK;
    ...
    ar->buffer   = new_buff;
    ar->capacity = ar->size;
    simuFlushOpt(&ar->buffer, sizeof(ar->buffer));
    simuFlushOpt(&ar->capacity, sizeof(ar->capacity));
    simuSfence();
    return CC_OK;
}
 ...
 */
__attribute__((used)) size_t cc_array_contains(CC_Array *ar, void *element)
{
    size_t o = 0;
    size_t i;
    for (i = 0; i < ar->size; i++) {
 ...
 */
__attribute__((used)) size_t cc_array_contains_value(CC_Array *ar, void *element, int (*cmp) (const void*, const void*))
{
    size_t o = 0;
    size_t i;
    for (i = 0; i < ar->size; i++) {
 ...
 */
__attribute__((used)) size_t cc_array_size(CC_Array *ar)
{
    return ar->size;
}
 ...
 */
__attribute__((used)) size_t cc_array_capacity(CC_Array *ar)
{
    return ar->capacity;
}
 ...
 *                before the first
 */
__attribute__((used)) void cc_array_sort(CC_Array *ar, int (*cmp) (const void*, const void*))
{
 ...
 */
__attribute__((used)) static enum cc_stat expand_capacity(CC_Array *ar)
{
 ...
 *               element
 */
__attribute__((used)) void cc_array_map(CC_Array *ar, void (*fn) (void *e))
{
    size_t i;
    for (i = 0; i < ar->size; i++)
        fn(ar->buffer[i]);
 ...
 */
__attribute__((used)) void cc_array_reduce(CC_Array *ar, void (*fn) (void*, void*, void*), void *result)
{
    if (ar->size == 1) {
 ...
 */
__attribute__((used)) void cc_array_iter_init(CC_ArrayIter *iter, CC_Array *ar)
{
    iter->ar    = ar;
    iter->index = 0;
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_iter_next(CC_ArrayIter *iter, void **out)
{
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_iter_remove(CC_ArrayIter *iter, void **out)
{
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_iter_add(CC_ArrayIter *iter, void *element)
{
 ...
 * CC_ERR_OUT_OF_RANGE.
 */
__attribute__((used)) enum  cc_stat cc_array_iter_replace(CC_ArrayIter *iter, void *element, void **out)
{
 ...
 * @return the index.
 */
__attribute__((used)) size_t cc_array_iter_index(CC_ArrayIter *iter)
{
    return iter->index - 1;
}
 ...
 */
__attribute__((used)) void cc_array_zip_iter_init(CC_ArrayZipIter *iter, CC_Array *ar1, CC_Array *ar2)
{
    iter->ar1 = ar1;
    iter->ar2 = ar2;
    iter->index = 0;
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_zip_iter_next(CC_ArrayZipIter *iter, void **out1, void **out2)
{
 ...
 * already removed.
 */
__attribute__((used)) enum  cc_stat cc_array_zip_iter_remove(CC_ArrayZipIter *iter, void **out1, void **out2)
{
 ...
 */
__attribute__((used)) enum  cc_stat cc_array_zip_iter_add(CC_ArrayZipIter *iter, void *e1, void *e2)
{
 ...
 * @return CC_OK if the element was successfully replaced, or CC_ERR_OUT_OF_RANGE.
 */
__attribute__((used)) enum  cc_stat cc_array_zip_iter_replace(CC_ArrayZipIter *iter, void *e1, void *e2, void **out1, void **out2)
{
    if ((iter->index - 1) >= iter->ar1->size || (iter->index - 1) >= iter->ar2->size)
        return CC_ERR_OUT_OF_RANGE;
 ...
 */
__attribute__((used)) size_t cc_array_zip_iter_index(CC_ArrayZipIter *iter)
{
    return iter->index - 1;
}
